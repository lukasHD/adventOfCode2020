...
def find_wrong_number(numbers, preamble, debug = False):
    ...
    for k, num in enumerate(numbers):
        if k < preamble:
            ...
            window.put(num)
            continue
        allowed_numbers = list(map(lambda pair: pair[0]+pair[1], combinations(list(window.queue), 2)))
        if debug: print("num {} in allowed? {}".format(num, allowed_numbers))
...
def find_weaknes(numbers, weakness):
    for window_length in range(2,50):
        ...
        for k, num in enumerate(numbers):
            ...
            if weakness == sum(current_numbers):
...
@Timer()
def run_part_2(in_file: str, weakness = 507622668, debug: bool = False) -> int:
    pretty.printHeader(DAY, 2, inspect.stack()[0].function, in_file)
    result = 0
    numbers = loadingUtils.importToIntArray(in_file)
    result = find_weaknes(numbers, weakness)
    print("Result = {}".format(result))
    return result
